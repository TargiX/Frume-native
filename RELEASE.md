...
| Planned next source version/build | `1.1.0 (10)`; confirm build `10` is unused before archive |
...
4. Reconcile the existing editable App Store version `1.0` with the archive
   marketing version `1.1.0` before upload. Use the exact version value Apple
...
- The command requires Xcode `27.0 (27A266a)` and verifies that `HEAD`, the
...
### Xcode compiler-discovery workaround

On the current release machine, the pinned Xcode's SwiftBuild service launches
...
The source defaults to build `10`. The historical TestFlight trains `1.0.0`,
