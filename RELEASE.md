...
2026-08-01. A `TODO` below is a hard release gate, not an optional cleanup.
...
| Public links | The live record still points Support to `https://frume.vercel.app/` and Privacy Policy to `https://frume.vercel.app/privacy`; those old photo-frame-era pages are not verified as accurate for the puzzle product. No replacement production URL has been approved. | Publish accurate public pages, set `EXPO_PUBLIC_PRIVACY_URL` and `EXPO_PUBLIC_SUPPORT_URL`, replace both dashboard fields, and verify the final URLs in-app and without authentication. |
...
| Dependency audit | Mobile production dependencies currently report 17 npm advisories (12 moderate, 5 high, 0 critical), all in the Expo/Node tooling chain; npm offers only a breaking Expo 57 upgrade for the remaining graph. The Worker workspace reports 0 advisories. | Do not run a blind `npm audit fix --force`. Record a reviewed SDK-upgrade/risk decision and repeat the audit from the exact release commit. |
